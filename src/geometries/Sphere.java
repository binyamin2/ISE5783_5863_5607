...
public class Sphere extends RadialGeometry{
    ...
    public List<Point> findIntersections(Ray ray) {
        ...
        if (alignZero(t1)>0){
            if (alignZero(t2)>0)
                return List.of(ray.getP0().add(ray.getV0().scale(t1)),ray.getP0().add(ray.getV0().scale(t2)));
            else
                return List.of(ray.getP0().add(ray.getV0().scale(t1)));
        }
        if (alignZero(t2)>0){
            return List.of(ray.getP0().add(ray.getV0().scale(t2)));
        }
    return null;
    }
}
